...
def load_digit38_mnist(root: str = "./data", download: bool = False) -> tuple[Digit38MNIST, Digit38MNIST]:
...
def split_train_validation(dataset: Digit38MNIST, seed: int, val_ratio: float = 0.2) -> tuple[Subset, Subset]:
...
def make_hue_offsets(num_official_images: int, seed: int, jitter: float = HUE_JITTER) -> torch.Tensor:
    """공식 MNIST 원본 index별 δ~Uniform(-5°, +5°)를 한 번만 생성한다."""
    generator = torch.Generator().manual_seed(seed)
    return torch.rand(num_official_images, generator=generator) * (2.0 * jitter) - jitter


def make_biased_color_ids(labels: torch.Tensor, p: float = 0.99, seed: int = 42) -> torch.Tensor:
...
def label_color_distribution(labels: torch.Tensor, color_ids: torch.Tensor) -> torch.Tensor:
...
def spurious_correlation(labels: torch.Tensor, color_ids: torch.Tensor) -> float:
    ...
    denominator = torch.sqrt(digit_centered.square().sum() * color_centered.square().sum())
    return float((digit_centered * color_centered).sum() / denominator)


def visualize_random_samples(dataset: SpuriousColoredMNIST, output_path=None, seed=42, show=False):
    """랜덤 표본 20개의 digit, target, color_id, 실제 Hue를 표시한다."""
    import matplotlib.pyplot as plt

    indices = torch.randperm(len(dataset), generator=torch.Generator().manual_seed(seed))[:20]
    fig, axes = plt.subplots(4, 5, figsize=(10, 8))
    for ax, idx in zip(axes.flat, indices.tolist()):
        image, target, digit, color_id, hue = dataset[idx]
        ax.imshow(image.permute(1, 2, 0))
        ax.set_title(f"digit={digit}, target={target}\ncolor={color_id}, H={hue:.1f}°", fontsize=8)
        ax.axis("off")
    fig.tight_layout()
    _finish_figure(fig, output_path, show)


def visualize_label_color_heatmap(labels, color_ids, output_path=None, show=False, title="Label-color distribution"):
...
def visualize_same_image_both_colors(subset: Subset, hue_offsets: torch.Tensor, subset_index=0,
...
def verify_dataset(seed: int = 42, p: float = 0.99, show_plots: bool = True) -> dict:
    ...
    print(f"train digit 3 / 8: {(labels == 3).sum().item():,} / {(labels == 8).sum().item():,}개")
    ...
    print(f"sample: shape={tuple(image.shape)}, target={target}, digit={digit}, "
    ...
    visualize_label_color_heatmap(
        labels, biased_ids, output / "biased_heatmap_38.png", show=show_plots,
        title=f"Biased digit-color distribution (p={p})",
    )
    visualize_label_color_heatmap(
        labels, neutral_ids, output / "neutral_heatmap_38.png", show=show_plots,
        title="Neutral digit-color distribution (p=0.5)",
    )
    visualize_same_image_both_colors(
        train_subset, hue_offsets,
        output_path=output / "same_image_both_colors.png", show=show_plots,
    )
