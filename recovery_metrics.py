...
def main() -> None:
    ...
    try:
        ...
        notes = [
            "M/Qは別々に報告し、加重合計は計算しません。",
            "M/Qは0~1、q_recovery_percentは%、*_ppはパーセントポイントです。",
            "null/CSV空欄は未計算・未定義です。回復率を0~100へ切り詰めません。",
            "基準モデルは同じ評価原本・色・モデル条件で、最初から中立データで学習してください。",
        ]
        # 出力説明はプロジェクトの使用言語に合わせる。
        notes = [
